...
SELECT DISTINCT
  	   EXTRACT(YEAR FROM start_date) AS entry_year,
...
SELECT _name 		AS project_name,
  ...
  FROM project
 WHERE
 	description ~ '[0-9]' OR description LIKE '%-%';
...
SELECT *
  FROM task
 WHERE EXTRACT(DAY FROM actual_end_date) >= EXTRACT(DAY FROM planned_end_date) + 3 AND
 	   EXTRACT(DAY FROM actual_end_date) <= EXTRACT(DAY FROM planned_end_date) + 10
ORDER BY
  project_id ASC,
  start_date DESC,
  _name DESC;
...
FROM project
WHERE
	(EXTRACT(YEAR FROM start_date) = EXTRACT(YEAR FROM CURRENT_DATE) OR
...
SELECT _name, start_date
  FROM project
 WHERE TRIM(description) LIKE '% % %' AND
       description NOT LIKE '%!%' AND
	   description NOT LIKE '%?%' AND
	   description NOT LIKE '%\%%' ESCAPE '\' AND
...
SELECT AVG(actual_end_date - start_date) AS average_duration
  FROM task
 WHERE project_id IN (2, 3, 6, 7, 8) AND
...
JOIN 
    project pr ON task.project_id = pr._id
WHERE 
    pr.short_name = 'ИПС BOOK' AND
    task.actual_end_date IS NULL
ORDER BY 
    e.surname,
    e._name,
    e.patronymic;
...
GROUP BY 
    project_name, project_short_name, project_manager
ORDER BY 
    project_name;
...
GROUP BY 
    manager._id, manager.surname, manager._name, manager.patronymic
HAVING 
    COUNT(project._id) > 1
ORDER BY 
    manager.surname;
...
SELECT 
    job_title._name AS job_title_name,
...
WHERE 
    EXTRACT(YEAR FROM job_history.start_date) = EXTRACT(YEAR FROM CURRENT_DATE)
GROUP BY 
    job_title_name
ORDER BY 
...
SELECT 
    EXTRACT(YEAR FROM project.start_date) AS start_year,
    project._name AS project_name,
    project.short_name AS project_short_name,
    COUNT(CASE WHEN task.actual_end_date IS NULL THEN 1 END) AS unfinished_tasks,
    COUNT(CASE WHEN task.actual_end_date IS NOT NULL AND task.actual_end_date <= task.planned_end_date THEN 1 END) AS completed_on_time,
    COUNT(CASE WHEN task.actual_end_date IS NOT NULL AND task.actual_end_date > task.planned_end_date THEN 1 END) AS completed_with_delay
FROM 
    project
LEFT JOIN 
    task ON project._id = task.project_id
GROUP BY 
    start_year, project_name, project_short_name
ORDER BY 
    start_year, project_name;
...
JOIN 
    task ON project._id = task.project_id
WHERE 
    EXTRACT(YEAR FROM project.start_date) < EXTRACT(YEAR FROM CURRENT_DATE) - 1 AND
    task.actual_end_date IS NULL
GROUP BY 
    project._id, project_name, project_short_name, project_start_date
HAVING 
    COUNT(task._id) > 3;
...
SELECT 
    DISTINCT tt._name AS task_type_name
FROM 
    task_type tt
JOIN 
...
JOIN employee e3 ON j3.employee_id = e3._id
JOIN job_title jt ON j1.job_title_id = jt._id
WHERE (j1.end_date IS NULL OR j1.end_date > CURRENT_DATE) AND
	  (j2.end_date IS NULL OR j2.end_date > CURRENT_DATE) AND
	  (j3.end_date IS NULL OR j3.end_date > CURRENT_DATE);
...
SELECT 
    youngest.surname AS youngest_surname,
...
WHERE youngest.date_of_birth = (
    SELECT MAX(date_of_birth)
    FROM employee
)
AND oldest.date_of_birth = (
...
HAVING COUNT(DISTINCT tp.employee_id) > (
    SELECT AVG(dev_count)
    FROM (
        SELECT COUNT(tp.employee_id) AS dev_count
        FROM task t
        JOIN task_participation tp ON t._id = tp.task_id
        GROUP BY t._id, t.project_id
    ) AS dev_count
);
...
WHERE NOT EXISTS (
    ...
    FROM task t
    JOIN task_participation tp ON t._id = tp.task_id
    WHERE tp.employee_id = e._id AND
		  t.actual_end_date <= t.planned_end_date
);
...
WHERE utc.unfinished_tasks_count = (
...
WITH CompletedTasksCount AS (
    SELECT tp.employee_id, COUNT(t._id) AS completed_tasks_count
    FROM task_participation tp
    JOIN task t ON tp.task_id = t._id
    JOIN project p ON t.project_id = p._id
    WHERE p._name = 'ИПС BOOK' AND t.actual_end_date IS NOT NULL
...
WHERE ctc.completed_tasks_count = (
    SELECT MAX(completed_tasks_count)
...
SELECT date_column
FROM (
    SELECT start_date AS date_column FROM job_history WHERE start_date IS NOT NULL
    UNION ALL
    SELECT end_date AS date_column FROM job_history WHERE end_date IS NOT NULL
    UNION ALL
    SELECT start_date AS date_column FROM project WHERE start_date IS NOT NULL
    UNION ALL
    SELECT start_date AS date_column FROM task WHERE start_date IS NOT NULL
) AS all_dates
